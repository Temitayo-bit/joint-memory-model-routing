...
- For every external-service operation, use the relevant MCP connector first when it supports the required action; otherwise use the service's first-party API or CLI. Use browser automation only as a last resort after verifying that the specific operation is unavailable through MCP, API, and CLI. Do not open or operate a dashboard merely to inspect status, logs, costs, resources, or other information that an MCP tool, API, or CLI can provide. When browser use is necessary, state which required capability is missing before opening it, limit browser activity to that capability, and return to MCP/API/CLI immediately afterward.
...
- After **every** experiment, give the user a concise findings rundown before moving on: the purpose and condition tested; the exact verified setup; results, failures, and missing cells; plain-language interpretation and limits; durable artifact locations; actual or pending costs and active resources; and the immediate next step. Clearly distinguish a smoke test, mock, or partial run from a completed experimental condition.
...
- Deploy the browser application on Vercel. Use Supabase Auth, RLS, and Edge Functions for authenticated memory access and the request controller; use RunPod only for the active self-hosted inference server. Vercel holds only public client configuration, never privileged Supabase or RunPod credentials. The RunPod proxy URL and inference credential belong only in Supabase Edge Function secrets. Narrow exception: the supervised local `--transport direct` workflow may hold those same values only as ephemeral operator environment variables (`TEXT_BASELINE_MODEL_BASE_URL`, `TEXT_BASELINE_MODEL_BEARER`) for that session; never commit, log, screenshot, or write them into artifacts.
...
When the user authorizes an ordered workflow with instructions such as “go,” “continue,” or “do everything,” Codex must carry out the authorized work through each successive checkpoint. Do not merely announce intended actions, stop after reporting a completed checkpoint, or ask whether to proceed to the already-authorized next step. Continue until the requested outcome is complete or a genuine blocker requires the user: missing authorization, a material scope-changing decision, a safety or credential boundary, an exhausted cost limit, or an external failure that cannot be resolved safely within scope. Continue providing concise progress updates during long-running work, but treat those updates as informational rather than stopping points.
