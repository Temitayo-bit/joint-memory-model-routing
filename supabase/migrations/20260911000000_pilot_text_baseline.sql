...
as $$
  with ranked as (
    select
      ...
      (1 - (i.embedding <=> p_query_embedding))::double precision as vector_similarity
...
grant select, insert, update on table public.pilot_request_results to authenticated;
grant select, insert on table public.pilot_call_log to authenticated;
